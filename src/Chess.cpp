...
void Chess::updateMoves(const string &newMoves) {
    moves=newMoves;
}
...
void Chess::excuteCastling() {
    int srcRow = m_input[0] - 'a'; // Source row (king location)
    int srcCol = m_input[1] - '1'; // Source column (king location)
    int destRow = m_input[2] - 'a'; // Destination row (rook location)
    int destCol = m_input[3] - '1'; // Destination column (rook location)

    // Determine if it's king-side or queen-side castling
    if (srcCol < destCol) { // King-side castling
        // Move the king
        m_boardString[srcRow * 8 + srcCol] = '#';
        m_boardString[srcRow * 8 + (srcCol + 2)] = m_turn ? 'K' : 'k';
        // Move the rook
        m_boardString[destRow * 8 + destCol] = '#';
        m_boardString[destRow * 8 + (srcCol + 1)] = m_turn ? 'R' : 'r';
...
void Chess::doTurn()
{
    ...
    switch (m_codeResponse)
    {
        ...
        case 44:
        {
            excute();
            if(m_turn) {
                m_msg = "Checkmate:) - Player 1 Wins (White)\nPress Key to exit the game";
                m_turn = !m_turn;
            }else{
                m_msg = "Checkmate:) - Player 2 Wins (black)\nPress Key to exit the game";
                m_turn = !m_turn;
            }
            displayBoard();
            exit(0);
